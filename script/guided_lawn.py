...
def turn_right(angle, time): #Rotate about the z axis
    r_angle = math.radians(angle)
    turn_rate = r_angle/time
    for j in range(time):
        forward_cmd.twist.angular.x = 0.0
...
def switch_modes_manual():
	print("\n----------switch_modes----------")
	rospy.wait_for_service("/mavros/set_mode")
	modes = rospy.ServiceProxy("/mavros/set_mode", SetMode)
	resp = modes(custom_mode ='MANUAL')
	rospy.sleep(2)
	return
...
def move_in_shape():
    ...
    armingCall()
    switch_modes()
    forward(20,2) #parameters: distance to move forward in meters, speed in meters/sec
    stop()
    turn_left(90,3) #parameters: turn angle in degrees, time required to turn in sec
    print("First left turn") #first left
    stop()
    forward(20,2)
    stop()
    turn_right(90, 3)
    print("Second right turn") #first right
    stop()
    forward(20,2)
    stop()
    turn_left(90, 3)
    ...
    stop()
    turn_left(90,3) #parameters: turn angle in degrees, time required to turn in sec
    print("Third left turn") #first left
    stop()
    forward(20,2)
    stop()
    turn_left(90, 3)
    print("Fourth right turn") #last right
    stop()
    forward(20,3)
    stop()
    #switch_modes_manual()
