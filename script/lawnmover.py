...
class Catabot:
    ...
    def myController_start(self):
        ...
        if self.direction_input == 1:
            ...
            left_motor = 1500
        '''
        self.right_motor = 1300
        self.left_motor = 1300
        self.right_motor_t = 1550
        self.left_motor_t = 1450
        self.right_motor_t1 = 1450
        self.left_motor_t1 = 1550
        ...
        try:
            while i<4:
                ...
                rospy.sleep(for_t)
                servoControlService = rospy.ServiceProxy('/mavros/cmd/command', mavros_msgs.srv.CommandLong) 
                servoControlService(command = 183, param1 = 1.0, param2 = self.right_motor_t1)
                servoControlService(command = 183, param1 = 3.0, param2 = self.left_motor_t1)
                rospy.sleep(turn_t)
                servoControlService = rospy.ServiceProxy('/mavros/cmd/command', mavros_msgs.srv.CommandLong) 
                servoControlService(command = 183, param1 = 1.0, param2 = self.right_motor)
                servoControlService(command = 183, param1 = 3.0, param2 = self.left_motor)
